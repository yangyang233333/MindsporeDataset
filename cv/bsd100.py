...
# 数据集下载链接
resources1 = r'https://www2.eecs.berkeley.edu/Research/Projects/CS/vision/bsds/'

# 数据集内的所有文件
resources2 = ["image_SRF_4",
              "image_SRF_3",
              "image_SRF_2", ]


class BSD100Generator:
    ...
    def read_image(self):
        ...
        content = sorted(glob.glob(os.path.join(root, filename) + r'\*LR.png'))
        return content

    def read_label(self):
        filename = f"image_SRF_{self.scale}"
        content = sorted(glob.glob(os.path.join(root, filename) + r'\*HR.png'))
        return content
